...
const create = async (req, res) => {
    try {
      ...
      // Parsear los campos JSON manualmente si existen
      const jsonFields = ['alimentacion', 'descanso', 'aseo', 'vestimenta', 'juego', 'conducta'];
      jsonFields.forEach(field => {
        if (req.body[field]) {
          try {
            req.body[field] = JSON.parse(req.body[field]);
          } catch (error) {
            console.warn(`Error al parsear ${field}:`, error.message);
            req.body[field] = null;
          }
        }
      });
